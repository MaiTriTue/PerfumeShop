export { default as Home } from '~/pages/Home';
export { default as ProDetailPage } from '~/pages/ProDetailPage';
export { default as HotBrand } from '~/pages/HotBrand';
export { default as BeautyManual } from '~/pages/BeautyManual';
export { default as MonthlyProgram } from '~/pages/MonthlyProgram';
export { default as Category } from '~/pages/Category';
export { default as Warning404 } from '~/pages/Warning404';
export { default as Login } from '~/pages/Login';
export { default as Register } from '~/pages/Register';
export { default as CartPage } from '~/pages/CartPage';

export { default as Blog } from '~/pages/Blog';
export { default as BlogPost } from '~/pages/BlogPost';
export { default as Contact } from '~/pages/Contact';
export { default as Endow } from '~/pages/Endow';
